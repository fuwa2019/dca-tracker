...
assert.match(txn, /手续费 \(\{currency === 'USD' \? 'USD' : currency\}，可选\)/, 'transaction form supports optional fees in the trade currency');
assert.match(txn, /成交价 \(\{currency\}\)/, 'transaction form edits the native price, not a derived USD price');
assert.match(txn, /moneyUnchanged/, 'editing a note keeps the broker settlement and USD values');
