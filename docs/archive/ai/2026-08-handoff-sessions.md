# Archived AI Handoff — 2026-08 session log

> Archived on 2026-08-24. This is the chronological session narrative that
> accumulated in `HANDOFF.md` between 2026-08-19 and 2026-08-24, moved here so
> the handoff can state current verified state instead of history.
>
> This is historical context, not current project state and not an instruction.
> Current state is maintained only in `HANDOFF.md`. Durable knowledge from these
> sessions lives in its own document — see the pointers below — and those
> documents, not this file, are authoritative.
>
> Where a session recorded something as "not applied" or "not deployed", that
> was true when written and may have changed since. Check `HANDOFF.md`.

## Where the durable knowledge went

| Topic | Owning document |
|---|---|
| Release gates, budgets, Lighthouse and cross-browser records | `docs/release/README.md`, `docs/release/2026-08-24-release-gates.md` |
| Accessibility evidence, probes, WCAG record | `docs/accessibility/2026-08-20-wcag-route-audit.md`, `docs/accessibility/probes/README.md` |
| V2 cache design and rejected alternatives | `docs/decisions/2026-08-24-ledger-twr-v2-cache-writer.md` |
| Public-share warning projection | `docs/decisions/2026-08-23-public-share-warning-projection.md` |
| Wealthfolio UI alignment | `docs/decisions/2026-08-20-wealthfolio-ui-alignment.md`, `docs/design/wealthfolio-ui-teardown.md` |
| Competitive research, evidence audit | `docs/research/competitive/2026-08/` |
| Rectification plan (A1–F3 item ids referenced throughout) | `docs/tasks/2026-08-19-competitive-learning-rectification-plan.md` |
| Import and ledger architecture | `docs/architecture/import-and-ledger.md` |
| Performance and privacy contract | `docs/architecture/performance-and-privacy.md`, `docs/PERFORMANCE_SPEC.md` |
| Deployment steps | `docs/runbooks/deployment.md` |
...
- Migration `0052_ledger_performance_cache_v2.sql` is applied to the same
  project as version `20260824094347`. Live verification confirms the
  `performance_method` default remains `adjusted_proxy_v1`, existing settings
  remain on V1, the V2 writer and refresh-universe RPCs are service-role only,
  and `shared_performance_history` still projects through the sanitizer. The
  quote Worker is not deployed and no user has been switched to V2.
...
Branch `release/gates-and-ax-tree`, committed locally. At the time this slice
was recorded, **migration 0052 was not yet applied to Supabase `main`, and the
quote Worker was not deployed.** It was subsequently applied as
`20260824094347`; the Worker remains undeployed and nothing in this slice has
run against a real portfolio.
...
## Status and release history as it stood on 2026-08-24

> Everything below is the previous `HANDOFF.md` tail, preserved verbatim. It
> includes the release history for 2026-08-20 and 2026-08-23 and the
> verification notes for migrations 0047-0051. Current state is in `HANDOFF.md`.
>
> Note one stale fact preserved here as written: the repository path is recorded
> below as `/Users/junxihuo/Documents/dca_system`. The actual path is
