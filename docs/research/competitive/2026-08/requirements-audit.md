...
| WCAG 2.2 AA for all named routes | partial | Ghostfolio authenticated Demo routes have no 390px overflow and its tablist route cycle is keyboard-operable; Wealthfolio narrow Activities/Appearance layouts expose forward/reverse Tab traversal, visible focus state and keyboard access to visible controls; Portfolio Performance narrow capture exposes clipped columns and no distinct focus state; product reduced-motion stylesheet rules are present where observed | Ghostfolio header/menu keyboard and actual reduced-motion emulation; automated/manual audit for login, overview, performance, share and settings; native reduced-motion runtime evidence and Portfolio Performance keyboard/responsive remediation |
