-- DCA Tracker — unified portfolio history RPC
--
-- Both authenticated dashboard history and anonymous shared history now use the
-- same internal TWR engine. Dashboard gets full NAV/PnL fields; share links only
-- get percentages.
...
begin
    ...
    while v_d <= v_today loop
        ...
        from public.cashflows
        where user_id = p_user_id and usd_in_date = v_d;
        ...
        for r in
            select ticker, side, shares, price, kind
            from public.transactions
        ...
        v_results := v_results || jsonb_build_array(v_point);
        v_d := v_d + 1;
    end loop;
...
begin
    ...
    return public._history_for_user(auth.uid(), true);
end;
$$;
...
begin
    ...
    return public._history_for_user(v_user_id, false);
end;
$$;
