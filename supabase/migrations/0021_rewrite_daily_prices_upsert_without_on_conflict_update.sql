-- DCA Tracker: rewrite daily_prices upsert without ON CONFLICT DO UPDATE.
--
-- Some database safe-update guards still reject ON CONFLICT DO UPDATE even when
-- the conflict action includes a WHERE clause. This keeps the service-role RPC
-- signature stable for the Worker while using an explicit UPDATE followed by an
...
begin
    ...
    where dp.ticker = d.ticker
      ...
      and (
        ...
        or dp.updated_at is distinct from d.updated_at
      );
