-- DCA Tracker - restrict performance history to US trading days.
--
-- The performance chart now emits points only on dates where SPY has an
-- actual persisted adjusted/close price. Weekend, holiday, and other
-- non-trading flat points are excluded from both the dashboard and share view.
...
begin
    ...
    if v_start is null then
        return jsonb_build_object(
            ...
            'date_basis', 'benchmark_price_dates',
            'trading_calendar', v_trading_calendar,
            'excluded_non_trading_days', true,
            'dirty', false,
            'warnings', '[]'::jsonb,
    ...
    return (
        ...
        calendar as (
            ...
            where ticker = v_trading_calendar
        ...
        shifted_txn_warnings as (
            select
                effective_date as date,
                jsonb_build_object(
                    'date', effective_date,
                    'type', 'non_trading_transaction_shifted',
                    'original_date', trade_date
                ) as warning
            from txns_with_effective_date
            where effective_date is not null
              and effective_date <> trade_date
            group by effective_date, trade_date
        ),
        missing_calendar_warnings as (
            select
                trade_date as date,
                jsonb_build_object(
                    'date', trade_date,
                    'type', 'missing_benchmark_trading_day',
                    'ticker', v_trading_calendar
                ) as warning
            from txns_with_effective_date
        ...
        warnings_json as (
            select coalesce(
                (
                    select jsonb_agg(warning order by date)
                    from (
                        select
                            date,
                            jsonb_build_object(
                        ...
                        select date, warning
                        from shifted_txn_warnings

                        union all

                        select date, warning
                        from missing_calendar_warnings
                    ) warning_rows
                ),
                '[]'::jsonb
            ) as warnings
