-- DCA Tracker — fix warnings aggregation in _performance_history_for_user_fast
--
-- The previous version selected warnings_arr as a scalar subquery column alongside
-- the cumulative rows, then wrapped it in max() to satisfy aggregation. That pattern
-- is fragile and can break depending on PG version / query-planner rewrites.
--
-- This rewrite separates concerns:
--   series_json   → jsonb_agg(series)   — one row
--   warnings_json → jsonb_agg(warnings) — one row
--   Final SELECT  → series_json CROSS JOIN warnings_json → jsonb_build_object
--
-- No GROUP BY, no max() hack, no mixed aggregation.
--
-- Sell validation (INSERT/UPDATE/DELETE with running-shares timeline) is already
-- covered by 0016_fix_transaction_timeline_and_twr.sql PART 1 — no changes needed here.
...
begin
    ...
    return (
        ...
        -- Collect days where a factor was invalid (non-positive or missing)
        -- so they can be surfaced as warnings instead of silently skipped.
        warnings_source as (
            select
                date,
                case
        ...
        -- Cumulative TWR: skip null factors (they contribute ln(1)=0, i.e. no change)
        cumulative as (
            select
                date,
                coalesce(
