...
function parseInput(input: ImportInput): IbkrParsedImport {
  ...
  for (let index = 0; index < body.length; index += 1) {
    ...
    const actionText = valueAt(row, columns, 'action') || description;
    const kind = actionKind(actionText);
    const ticker = normalizeTicker(valueAt(row, columns, 'symbol'));
    const date = parseDate(valueAt(row, columns, 'date'));
    const currency = (valueAt(row, columns, 'currency') || 'USD').toUpperCase();
    const amount = parseDecimal(valueAt(row, columns, 'amount'), 10);
    const usdAmount = parseDecimal(valueAt(row, columns, 'usd_amount'), 10);
    ...
    if (kind === 'buy' || kind === 'sell') {
      const shares = parseDecimal(valueAt(row, columns, 'quantity'), 10);
      const price = parseDecimal(valueAt(row, columns, 'price'), 12);
      const parsedFees = parseDecimal(valueAt(row, columns, 'fees'), 10) ?? '0';
